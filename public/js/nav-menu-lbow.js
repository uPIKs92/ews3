...
const listBtnSetpsa = [
	['areano', ''], ['add', ''], ['BTM1', ''], ['min-km', ''], ['btm2', ''], ['max-km', ''], ['On', ''], ['', ''], ['', ''], ['', ''],
	['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''],
	['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''],
	['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''],
	['abort', ''], ['.', ''], ['home', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['bakspc', ''], ['dltfld', ''], ['enter', '']
	...
	if (buttonName != '' || buttonName != null) {
		switch (buttonName) {
			...
			case 'creftr': {
				...
				hideNumBtn('#btnCreftr-', 30, 40);
				disableBtnNumber('#btnCreftr-');
				delChar('#btnCreftr-47, #btnCreftr-48');
				return;
			}
			case 'crevec': {
				let count = 0;
			...
			case 'crearc': {
				...
				$('.cursor i').removeClass('d-none');
				enableBtnNumber('#crearc-menu-btn li', listBtnCrearc, 'btnCrearc-');
				delChar('#btnCrearc-47, #btnCrearc-48');
				return;
			}
			case 'ppibrt': {
				let count = 0;
					...
					if (listBtnSettrk[id][0] != '') {
						...
						switch (id) {
							...
							case 4:
								...
								showNumBtn('#btnSettrk-', 30, 40);
								enableBtnNumber('#settrk-menu-btn li', listBtnSettrk, 'btnSettrk-');
								break;
							case 6:
								console.log(id + ' fnSettrk called');
								event.stopImmediatePropagation();
			...
			case 'histry': {
				...
				$('.cursor i').removeClass('d-none');
				delChar('#btnHistry-47, #btnHistry-48');
				return;
			}
			case 'radplt': {
			...
			case 'setcps': {
					...
					if (listBtnSetcps[id][0] != '') {
						...
						switch (id) {
							...
							case 1:
								...
								$('#btnSetcps-' + id).children('span').each(function () {
									this.className = btn[($.inArray(this.className, btn) + 1) % btn.length];

									switch ($(this).attr('class')) {
										case btn[0].toString():
											singleToggleBtn(1, this, btn[0], '#tg-0');
											break;
										case btn[1].toString():
											singleToggleBtn(1, this, btn[1], '#tg-1');
											break;
										case btn[2].toString():
											singleToggleBtn(1, this, btn[2], '#tg-2');
											break;
									}
								});

								showNumBtn('#btnSetcps-', 30, 40);
								enableBtnNumber('#setcps-menu-btn li', listBtnSetcps, 'btnSetcps-');
								break;
							case 40:
								console.log(id + ' fnLine called');
								event.stopImmediatePropagation();
				...
				delChar('#btnSetcps-47, #btnSetcps-48');
				return;
			}
			case 'setpsa': {
				return;
			...
			case 'pltsel': {
				...
				count = 2;
				$('.cursor i').removeClass('d-none');
				delChar('#btnPltsel-47, #btnPltsel-48');
				return;
			}
		}
	}
