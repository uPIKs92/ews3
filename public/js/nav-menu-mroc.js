...
function createButton(buttonName, lastCount) {
	...
	if (buttonName != '' || buttonName != null) {
		lastCount = ListMenu(buttonName, countIdx, lastCount);
		return;
	}
}

function ListMenu(buttonName, countIdx, lastCount) {
	switch (buttonName) {
		case 'main': {
					...
					switch (id) {
						...
						case 49:
							...
							if ($('#input-VK').val() != '') {
								...
								switch (count) {
									case 1:
										if (parseInt($('#input-VK').val()) < 1 || parseInt($('#input-VK').val()) > 4) {
					...
					switch (id) {
						...
						case 49:
							...
							if ($('#input-VK').val() != '') {
								...
								switch (count) {
									case 1:
										if (parseInt($('#input-VK').val()) < 1 || parseInt($('#input-VK').val()) > 6) {
										...
										break;
									case 2:
										if (parseInt($('#input-VK').val()) < 1 || parseInt($('#input-VK').val()) > 8) {
									...
									case 6:
										...
										if (parseInt($('#input-VK').val()) < 1 || parseInt($('#input-VK').val()) > 20) {
									...
									case 5:
										...
										setTimeout(() => {
											if ($('#treeNum-5').text().length == 7) {
												...
												printMessage('INVALID ' + $('#btnUpdcde-' + count).text() + ' VALUE');
											}
										}, 125);
										break;
								}
								...
								switch (count) {
									case 1:
										if (parseInt($('#input-VK').val()) > 360) {
											printMessage($('#input-btn-out').text() + ' value should be between 0-360');
										} else {
											count++;
		...
		case 'updtrk': {
				...
				if (listBtnUpdtrk[id][0] != '') {
					...
					switch (id) {
						case 1:
							...
							setTimeout(function() {
								showNumBtn('#btnUpdtrk-', 30, 41, listBtnUpdtrk);
			...
			showToggle(toggLabelB1, toggLabelB2);
			showNumBtn('#btnUpdtrk-', 30, 41, listBtnUpdtrk, 1);
			callAlphaVK('#btnUpdtrk-');
		...
		case 'swap': {
			...
			showNumBtn('#btnSwap-', 30, 41, listBtnSwap, 1);
			callAlphaVK('#btnSwap-');
			return lastCount;
		}
		case 'trksrc': {
		...
		case 'radstr': {
			...
			showNumBtn('#btnRadstr-', 30, 41, listBtnRadstr, 1);
			return lastCount;
		}
		case 'trkref': {
			let count = 1;
			...
			showNumBtn('#btnTrkref-', 30, 41, listBtnTrkref, 1);
		...
		case 'merge': {
			...
			showNumBtn('#btnMerge-', 30, 41, listBtnMerge, 1);
			callAlphaVK('#btnSwap-');
			return lastCount;
		}
		case 'setnac': {
				...
				if (listBtnSetnac[id][0] != '') {
					...
					switch (id) {
						...
						case 2:
							...
							showNumBtn('#btnSetnac-', 30, 41, listBtnSetnac);
							break;
						case 3:
							console.log(id + ' fnSetnac called');
							event.stopImmediatePropagation();
				...
				if (listBtnSetnai[id][0] != '') {
					...
					switch (id) {
						...
						case 1:
							...
							showNumBtn('#btnSetnai-', 30, 41, listBtnSetnai);
							break;
						case 2:
							console.log(id + ' fnSetnai called');
							event.stopImmediatePropagation();
		...
		case 'corr': {
			...
			showNumBtn('#btnCorr-', 30, 41, listBtnCorr, 1);
			callAlphaVK('#btnSwap-');
			return lastCount;
		}
		case 'airmov': {
				...
				if (listBtnAdsfp[id][0] != '') {
					...
					switch (id) {
						...
						case 15:
							...
							showNumBtn('#btnAdsfp-', 30, 41, listBtnAdsfp);
							count = id + 2;
							break;
						case 21:
							console.log(id + ' fnAdsfp called | ' + count);
				...
				if (listBtnLeg[id][0] != '') {
					...
					switch (id) {
						...
						case 5:
							...
							$('#three-toggle').empty();
							hideAlphaBtn('#btnLeg-');
							showNumBtn('#btnLeg-', 30, 41, listBtnLeg);
							break;
						case 40:
							console.log(id + ' fnLeg called');
							//event.stopImmediatePropagation();
...
function isGeoLat(state, toggLabelA, toggLabelB, inputVal) {
	...
	if (inputVal != '' || inputVal != undefined) {
		({ getInput, charCount, numCount, latCount, longCount } = geoLatCheck(
			state,
			getInput,
			inputVal,
			charCount,
...
function geoLatCheck(
	...
	return { getInput, charCount, numCount, latCount, longCount };
}
